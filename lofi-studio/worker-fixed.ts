...
app.post('/api/generate-music', async (c) => {
  ...
  try {
    ...
    const actualWorkId = data.workId || workId
...
// Serve the same React app (keeping the UI the same)
app.get('*', (c) => {
        ...
        const checkStatus = async () => {
          try {
            ...
            if (data.status === 'completed') {
              ...
              alert('Generation timed out. Use the Refresh Stuck button to check status.');
              fetchSongs();
            }
          } catch (error) {
