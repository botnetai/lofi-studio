...
app.get('/api/generate-music-status', async (c) => {
  ...
  console.log('Status response:', JSON.stringify(data).substring(0, 200))
    ...
    if (completedCount > 0) {
      ...
      if (result) {
        ...
        try {
          ...
          const verification = await c.env.R2.get(audioKey)
          console.log('Verification - File exists:', !!verification, 'Size:', verification?.size)
    ...
    if (metadata.workId) {
      try {
        ...
        if (data.data && data.data.response_data) {
          ...
          errors.push(`${song.name}: No data found`)
        }
      } catch (error) {
...
app.get('/test-audio/:songId', async (c) => {
  const songId = c.req.param('songId')
  ...
  const fullUrl = `https://lofi-studio.botnet-599.workers.dev${url}`
  
  return c.html(`<!DOCTYPE html>
<html>
<head>
  <title>Audio Test - ${song.name}</title>
</head>
<body style="background: #000; color: #fff; padding: 20px; font-family: sans-serif;">
  ...
  <h2>Direct Link:</h2>
  <a href="${url}" style="color: #8b5cf6;">Download/Play ${song.name}</a>
  
  <h2>Debug Info:</h2>
  <pre id="debug" style="background: #333; padding: 10px; overflow: auto;"></pre>
  ...
  <script>
    ...
    audio.addEventListener('loadstart', () => log('loadstart event'));
    audio.addEventListener('loadedmetadata', () => log('loadedmetadata event'));
    audio.addEventListener('canplay', () => log('canplay event'));
    audio.addEventListener('canplaythrough', () => log('canplaythrough event'));
    audio.addEventListener('error', (e) => {
      log('ERROR event:');
      log('- error code: ' + (audio.error ? audio.error.code : 'none'));
      log('- error message: ' + (audio.error ? audio.error.message : 'none'));
      
      if (audio.error) {
        switch(audio.error.code) {
          case 1: log('- MEDIA_ERR_ABORTED'); break;
          case 2: log('- MEDIA_ERR_NETWORK'); break;
          case 3: log('- MEDIA_ERR_DECODE'); break;
          case 4: log('- MEDIA_ERR_SRC_NOT_SUPPORTED'); break;
        }
      }
    });
...
// Test endpoint to check if file exists and can be played
app.get('/api/test-audio/:songId', async (c) => {
  const songId = c.req.param('songId')
  ...
  if (!song) {
    return c.json({ error: 'Song not found in database' })
  }
  ...
  // Try to re-download if missing and we have the original URL
  let redownloaded = false
  if (!object && song.metadata) {
    const metadata = JSON.parse(song.metadata as string)
    if (metadata.audio_url) {
      try {
        console.log('File missing, attempting to re-download from:', metadata.audio_url)
        const audioResponse = await fetch(metadata.audio_url)
        if (audioResponse.ok) {
          const audioBlob = await audioResponse.blob()
          await c.env.R2.put(key, audioBlob.stream(), {
            httpMetadata: { 
              contentType: 'audio/mpeg',
              contentLength: audioBlob.size.toString()
            }
  ...
  // Check again if re-download succeeded
  const finalCheck = await c.env.R2.get(key)
  ...
  return c.json({
    ...
    key,
    exists: !!finalCheck,
    size: finalCheck?.size,
    httpMetadata: finalCheck?.httpMetadata,
    status: song.status,
    redownloaded,
    metadata: JSON.parse(song.metadata as string || '{}')
  })
})
...
// Serve R2 files - Fixed with proper streaming
app.get('/files/*', async (c) => {
  ...
  try {
    ...
    if (!object) {
      ...
      // Try to find the song and re-download if possible
      const songId = key.replace('songs/', '').replace('.mp3', '')
      const song = await c.env.DB.prepare(
        "SELECT * FROM songs WHERE id = ?"
      ).bind(songId).first()
      ...
      if (song && song.metadata) {
        const metadata = JSON.parse(song.metadata as string)
        if (metadata.audio_url) {
          console.log('Attempting to re-download missing file from:', metadata.audio_url)
          try {
            const audioResponse = await fetch(metadata.audio_url)
            if (audioResponse.ok) {
              const audioBlob = await audioResponse.blob()
              ...
              // Try again
              const newObject = await c.env.R2.get(key)
              if (newObject) {
                console.log('Re-download successful, serving file')
                const headers = new Headers()
                headers.set('Content-Type', 'audio/mpeg')
                headers.set('Access-Control-Allow-Origin', '*')
                headers.set('Cache-Control', 'no-cache')
                return c.body(newObject.body, { headers })
              }
            }
          } catch (e) {
    ...
    // Always set audio/mpeg for MP3 files
    headers.set('Content-Type', 'audio/mpeg')
    ...
    // CORS headers - very permissive for debugging
    headers.set('Access-Control-Allow-Origin', '*')
    ...
    console.log('Serving with headers:', Object.fromEntries(headers.entries()))
...
<body class="bg-black text-white antialiased">
  ...
  <script type="text/babel">
    ...
    function App() {
      ...
      const testAudio = async (songId) => {
        try {
          const res = await fetch(\`/api/test-audio/\${songId}\`);
          const data = await res.json();
          console.log('Audio test result:', data);
          
          if (!data.exists) {
            if (data.redownloaded) {
              return { success: true, message: 'File was missing but has been re-downloaded. Try playing again.' };
            } else {
              return { success: false, message: 'Audio file not found in storage' };
            }
          }
      ...
      const togglePlay = async (songId, songUrl) => {
        if (playingSongId === songId) {
          ...
          try {
            ...
            const testResult = await testAudio(songId);
            if (!testResult.success) {
              setPlaybackErrors(prev => ({ ...prev, [songId]: testResult.message }));
              return;
            }
            
            if (testResult.message) {
              setPlaybackErrors(prev => ({ ...prev, [songId]: testResult.message }));
              // Wait a bit then try to play if file was re-downloaded
              if (testResult.message.includes('re-downloaded')) {
                setTimeout(() => togglePlay(songId, songUrl), 1000);
                return;
              }
            }
            ...
            audioRef.current.onerror = (e) => {
              const errorMessage = 'Cannot play audio. The file may still be processing.';
